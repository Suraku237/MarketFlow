# MarketFlow — SmartSchool

Student Records, Fee Invoicing & Staff Payroll Management System.
...
[`documentation/SmartSchool_Software_Design_Document.pdf`](documentation/SmartSchool_Software_Design_Document.pdf).
...
services/
  ...
  academic-service/      Week 3 module service: courses, enrollments, grades
...
documentation/
  ...
  SmartSchool_Software_Design_Document.pdf      Same document, exported
...
docker-compose.yml         Brings up MySQL, auth-service, academic-service and the Gateway
```
...
first time its data volume is created), `auth-service`, `academic-service`,
...
[`services/academic-service/README.md`](services/academic-service/README.md)
...
curl http://localhost:8081/api/v1/courses -H "Authorization: Bearer $TOKEN"   # -> academic-service
```
...
## Week 4: async messaging, security & load testing

```
curl http://localhost:8081/api/v1/finance/invoices -H "Authorization: Bearer $TOKEN"   # -> finance-service
```

Enrolling a student (`POST /api/v1/enrollments`) publishes an
`enrollment.created` event on RabbitMQ; `finance-service` consumes it
independently and creates an invoice, without the enrollment request waiting
on it. See [`services/finance-service/README.md`](services/finance-service/README.md)
for the event payload and live-demo steps, [`SECURITY.md`](SECURITY.md) for
the OWASP Top 10 write-up, and [`load-testing/README.md`](load-testing/README.md)
for the k6 load test.
...
docker compose exec db mysql -uroot -pdev-only-change-me smartschool
mysql> SHOW TABLES;
...
`idx_enrollments_student`) with `type` `ref`/`range` means the index was
used; `type: ALL` with `key: NULL` would mean a full table scan.
...
# Simulate data loss
docker compose exec db mysql -uroot -pdev-only-change-me smartschool -e "DELETE FROM courses;"
...
- A single combined **Entity-Relationship Diagram** (all tables across the
  Academic, Finance, and HR modules), authored in PlantUML (`er_diagram.puml`).
- The full **relational schema**, table by table, with types and constraints.
- The **3NF normalization rationale**, including the deliberate
  "denormalized-looking" column (`invoices.amount` as a fee snapshot at
  enrollment time) and why it doesn't violate 3NF.
- The **indexing strategy** across the schema, more than the 2-index minimum
  required.
