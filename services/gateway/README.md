...
via env var across all services). If verification fails, the Gateway
...
Deliberately out of scope for the Gateway: **role** checks (Admin vs Teacher
vs Student). That stays with whichever service owns the resource
(`authorize('admin')` in `academic-service`'s `POST /api/courses`, same
pattern as Week 1's `/api/reports` in `auth-service`) — the Gateway only
answers "is this a logged-in user at all", not "are they allowed to do this
specific thing". That's a deliberate separation: the Gateway is generic
infrastructure, role rules are business logic that belongs to the service
that owns the data.
...
the Gateway (both `auth-service`'s and `academic-service`'s), built from the
OpenAPI specs in [`openapi/`](openapi/). Open
...
TOKEN=$(curl -s -X POST localhost:8081/api/v1/auth/login -H "Content-Type: application/json" \
  -d '{"email":"admin@smartschool.test","password":"adminpass123"}' | node -e "process.stdin.on('data',d=>console.log(JSON.parse(d).token))")
...
curl -s localhost:8081/api/v1/courses -H "Authorization: Bearer $TOKEN"   # -> academic-service
