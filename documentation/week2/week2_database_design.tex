...
The schema targets \textbf{MySQL 8}, run as a Docker container (service
\texttt{db} in the repository's \texttt{docker-compose.yml}) alongside the
Week 1 authentication service. Fixed value-sets (\texttt{role},
\texttt{status}, \texttt{movement\_type}, \texttt{method}) use native
\texttt{ENUM} types; currency columns use \texttt{DECIMAL(10,2)} rather than
a floating-point type, to avoid rounding error on monetary values.
...
name & VARCHAR(255) & NOT NULL \\
category\_id & INT & NOT NULL, FK $\rightarrow$ categories(id) \\
supplier\_id & INT & FK $\rightarrow$ suppliers(id), nullable \\
unit\_price & DECIMAL(10,2) & NOT NULL, CHECK $\geq 0$ \\
quantity\_in\_stock & INT & NOT NULL, DEFAULT 0, CHECK $\geq 0$ \\
reorder\_level & INT & NOT NULL, DEFAULT 10 \\
created\_at & DATETIME & NOT NULL, DEFAULT CURRENT\_TIMESTAMP \\
...
\schemahead
id & INT & PRIMARY KEY AUTO\_INCREMENT \\
product\_id & INT & NOT NULL, FK $\rightarrow$ products(id) \\
change\_qty & INT & NOT NULL (signed: positive = in, negative = out) \\
movement\_type & ENUM & NOT NULL, ('RECEIPT','SALE','ADJUSTMENT','RETURN') \\
reference & VARCHAR(100) & e.g. purchase order or sale ID \\
created\_by & INT & FK $\rightarrow$ users(id), nullable \\
created\_at & DATETIME & NOT NULL, DEFAULT CURRENT\_TIMESTAMP \\
\bottomrule
\end{tabular}
\end{center}
...
\schemahead
id & INT & PRIMARY KEY AUTO\_INCREMENT \\
cashier\_id & INT & NOT NULL, FK $\rightarrow$ users(id) \\
sale\_date & DATETIME & NOT NULL, DEFAULT CURRENT\_TIMESTAMP \\
total\_amount & DECIMAL(10,2) & NOT NULL, DEFAULT 0, CHECK $\geq 0$ \\
...
\schemahead
id & INT & PRIMARY KEY AUTO\_INCREMENT \\
sale\_id & INT & NOT NULL, FK $\rightarrow$ sales(id) \\
product\_id & INT & NOT NULL, FK $\rightarrow$ products(id) \\
quantity & INT & NOT NULL, CHECK $> 0$ \\
unit\_price & DECIMAL(10,2) & NOT NULL, CHECK $\geq 0$ (price snapshot at sale time) \\
subtotal & DECIMAL(10,2) & NOT NULL, CHECK $\geq 0$ \\
\bottomrule
\end{tabular}
\end{center}
...
\schemahead
id & INT & PRIMARY KEY AUTO\_INCREMENT \\
sale\_id & INT & NOT NULL, UNIQUE, FK $\rightarrow$ sales(id) (1:1) \\
momo\_reference & VARCHAR(100) & mobile money gateway reference \\
amount & DECIMAL(10,2) & NOT NULL, CHECK $\geq 0$ \\
method & ENUM & NOT NULL, DEFAULT 'momo', ('momo','cash','card') \\
status & ENUM & NOT NULL, DEFAULT 'pending', ('pending','approved','declined') \\
paid\_at & DATETIME & nullable \\
\bottomrule
\end{tabular}
\end{center}
...
id & INT & PRIMARY KEY AUTO\_INCREMENT \\
product\_id & INT & NOT NULL, FK $\rightarrow$ products(id) \\
forecast\_date & DATE & NOT NULL \\
predicted\_quantity & INT & NOT NULL, CHECK $\geq 0$ \\
model\_version & VARCHAR(20) & NOT NULL, DEFAULT 'v1' \\
created\_at & DATETIME & NOT NULL, DEFAULT CURRENT\_TIMESTAMP \\
\bottomrule
\end{tabular}
\end{center}
...
password\_hash & VARCHAR(255) & NOT NULL (bcrypt hash, from Week 1 auth-service) \\
role & ENUM & NOT NULL, ('admin','cashier') \\
created\_at & DATETIME & NOT NULL, DEFAULT CURRENT\_TIMESTAMP \\
\bottomrule
\end{tabular}
\end{center}
...
run\_date & DATETIME & NOT NULL, DEFAULT CURRENT\_TIMESTAMP \\
status & ENUM & NOT NULL, DEFAULT 'draft', ('draft','processed','paid') \\
created\_by & INT & FK $\rightarrow$ users(id), nullable \\
\bottomrule
\end{tabular}
\end{center}
...
\schemahead
id & INT & PRIMARY KEY AUTO\_INCREMENT \\
payroll\_run\_id & INT & NOT NULL, FK $\rightarrow$ payroll\_runs(id) \\
staff\_id & INT & NOT NULL, FK $\rightarrow$ users(id) \\
base\_salary & DECIMAL(10,2) & NOT NULL, CHECK $\geq 0$ \\
deductions & DECIMAL(10,2) & NOT NULL, DEFAULT 0, CHECK $\geq 0$ \\
net\_pay & DECIMAL(10,2) & NOT NULL, CHECK $\geq 0$ \\
momo\_reference & VARCHAR(100) & payout reference \\
...
Run \texttt{database/scripts/explain\_query.sh} to print MySQL's
\texttt{EXPLAIN} output for two sample queries. The \texttt{key} column
names the index MySQL's optimizer actually chose (e.g.
\texttt{idx\_products\_name}), and \texttt{type} shows the access method
(\texttt{ref}/\texttt{range} meaning an index was used, versus \texttt{ALL}
with \texttt{key: NULL}, which would mean a full table scan).

% ================= CHAPTER 6: BACKUP & RESTORE =================
\chapter{Backup and Restore Procedure}

\section{Approach}
\texttt{mysqldump} is used to take a logical, consistent backup (a plain SQL
file of \texttt{DROP TABLE}/\texttt{CREATE TABLE}/\texttt{INSERT}
statements), and \texttt{mysql} reloads it. Four scripts under
\texttt{database/scripts/}, run via \texttt{docker compose exec} against the
\texttt{db} container, automate this for the live evaluation:

\begin{enumerate}[leftmargin=1.4em]
  \item \texttt{init\_db.sh} -- drops and recreates the \texttt{smartstock}
  database from \texttt{schema.sql} and loads \texttt{seed.sql}.
  \item \texttt{backup\_db.sh} -- writes a timestamped \texttt{mysqldump} to
  ...
  \item \texttt{explain\_query.sh} -- runs the two sample queries above
  through \texttt{EXPLAIN}.
\end{enumerate}
...
\begin{enumerate}[leftmargin=1.4em]
  ...
  \item Delete data on purpose, e.g.\ \texttt{docker compose exec db mysql -uroot -p... smartstock -e "DELETE FROM products;"}
